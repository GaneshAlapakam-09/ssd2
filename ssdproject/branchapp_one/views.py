...
from ssdapp.models import (
    ProductMaster, CategoriesMaster, CostMaster, Employee
)
...
from .decorators import branch_one_required
...
def signin(request):
    if request.method == 'POST':
        ...
        Office_Branch = "branch_one"
        user = authenticate(username=upperuser, password=password)
        if user is not None:
            if user.Office_Branch == Office_Branch:
                login(request, user)
                return redirect('branchapp_one:dashboard')
            else:
                messages.info(request, "This user is not belongs to Branch One")
                return redirect('branchapp_one:signin')
...
@login_required(login_url='branchapp_one:signin')
@branch_one_required
...
def addCustomer(request):
    if request.method == 'POST':
        ...
        altPhone = request.POST['alt_phone'].strip() or 000
        # Set to None if empty
        email = request.POST['email'].strip() or 'None'
        ...
        last_Agent = CustomerMaster.objects.filter(
...
def addMaterial(request):
    if request.method == "POST":
        ...
        if last_material:
            # Extract the numeric part
            last_id = int(last_material.Material_Id[3:])
...
def select_material(request):
    if request.method == 'POST':
        ...
        if last_Sno:
            # Assuming 'Sno' is the integer field storing serial numbers
            new_sno = last_Sno.S_No + 1
        else:
        ...
        if last_inward:
            # Extract the numeric part
            last_id = int(last_inward.Inward_Id[3:])
        ...
        Material = MaterialMaster.objects.filter(
            Material_Id=material_id, Status=1)
        if Material:
            InwardMaster.objects.create(
                S_No=new_sno,
                Inward_Id=new_inw_id,
                Material_Id=material_id,
                Material_Name=name.Material_Name,
                Date=date,
                Vendor_Name=vendor_name.upper(),
                Vendor_Mobile=vendor_phone,
                Vendor_GST=vendor_gst.upper(),
            ...
            return redirect('branchapp_one:select_material')
        return redirect('branchapp_one:listinward')
...
def add_outward(request, id):
    if request.method == "POST":
        ...
        if last_outward:
            # Extract the numeric part
            last_id = int(last_outward.Outward_Id[3:])
        ...
        return redirect('branchapp_one:listoutward')
...
def addProduct(request):
    ...
    if request.method == 'POST':
        ...
        return redirect('branchapp_one:addproduct')
...
def editProduct(request, id):
    if request.method == 'POST':
        ...
        ProductMaster.objects.filter(
            Product_Id=id).update(
            Date=date,
            Product_Name=name.upper(),
            GST=gst,
            HSN_Code=hsn.upper())

        return redirect('branchapp_one:listproduct')
...
def deleteProduct(request, id):
    ...
    if existing_product:
        ...
        CategoriesMaster.objects.filter(
...
def addCategories(request):
    if request.method == 'POST':
        ...
        CategoriesMaster.objects.create(
            Categories_Id=new_id,
            Product_Name=product_name,
            Categories_Name=category_name.upper(),
            Sub_Categories=sub_category.upper())

        return redirect('branchapp_one:addcategories')
...
def editCategories(request, id):
    if request.method == 'POST':
        ...
        CategoriesMaster.objects.filter(
            Categories_Id=id).update(
            Product_Name=product_name,
            Categories_Name=category_name.upper(),
            Sub_Categories=sub_category.upper())

        return redirect('branchapp_one:listcategories')
...
def add_Cost(request):
    if request.method == "POST":
        ...
        if cost_calculate == "Unit Cost":
            ...
            return redirect('branchapp_one:addcost')
...
def editCost(request, id):
    if request.method == "POST":
        ...
        CostMaster.objects.filter(Cost_Id=id).update(
            Cost_Id=id,
            Product_Name=product_name,
            Date=date,
            Category_Name=category_name,
        ...
        return redirect('branchapp_one:listcost')
        ...
        try:
            ...
            for entry in entries:
                ...
                detail_id = BillingMaster.objects.create(
                    ...
                    Customer_Name=data.get("customer_name") if data.get(
                        "customer_name") else data.get("agent_name"),
                    Phone_No=data.get("customer_phone") if data.get(
                        "customer_phone") else data.get("agent_phone"),
                    Date=data.get("bill_date"),
                    Grand_Total=int(data.get("grand_total", 0)) if data.get(
                        "grand_total") not in [None, "", "None"] else 0,
                    Grand_Total_With_Gst=0,
                    Added_By=request.user.username,
                    Pending_Amount=int(data.get("grand_total", 0)) if data.get(
                        "grand_total") not in [None, "", "None"] else 0,
                )
            ...
            for entry in entries:
                ...
                costs = entry.get("cost", "NONE") if entry.get(
                    "cost") not in [None, "", "None"] else "NONE"
                charge = entry.get("charges", 0) if entry.get(
                    "charges") not in [None, "", 0] else 0

                manual_amount = 0

                dimetions = entry.get("dimension")
                dimention_parts = dimetions.split("X")
                length, height = float(dimention_parts[0].strip()), float(
                    dimention_parts[1].split("=")[0].strip())
                ...
                whatsData = BillingDetails.objects.create(
                    ...
                    Custom_Product=entry.get("custom", "NONE") if entry.get(
                        "custom") not in [None, "", "None"] else "NONE",
                    Category_Name=splitted_spec[1],
                    Sub_Category=splitted_spec[2],
                    Size=entry.get("size", "NONE") if entry.get(
                        "size") not in [None, "", "None"] else "NONE",
                    Length=length,
                    Height=height,
                    Total_Sqft=float(entry.get("total_sqft", 0) if entry.get(
                        "total_sqft") not in [None, "", "None"] else 0),
                    # Convert to int (default 0 if missing)
                    Quantity=int(entry.get("quantity", 0)),
                    Cost_Per_Sqft=float(entry.get("cost_per_sqft", 0)),
                    Actual_Cost=float(costs) - float(charge),
                    Modified_Cost=manual_amount,
                    GST=float(entry.get("gst", 0)) if entry.get("gst") not in [
                        # Convert to float (default 0 if missing)
                        None, "", "None"] else 0,
                    HSN_Code=entry.get("hsn", "NONE") if entry.get(
                        "hsn") not in [None, "", "None"] else "NONE",
                    # Convert to float
                    Total_Cost=float(costs) - float(charge),
                    Total_Cost_With_Gst=float(
                        entry.get("total_with_gst", 0)),  # Convert to float
                    Remarks=entry.get("remarks", "NONE") if entry.get(
                        "remarks") not in [None, "", "None"] else "NONE",
                    Additional_Charges=entry.get("charges", 0) if entry.get(
                        "charges") not in [None, "", 0] else 0,
                    Difference_Amount=int(float(entry.get("cost", "NONE"))) if entry.get(
                        "cost") not in [None, "", "None"] else 0
                )

            return redirect("branchapp_one:listbill")
    ...
    if request.method == "POST":
        try:
            ...
            for entry in entries:
                ...
                if last_bill:
                    # Extract the numeric part
                    last_id = int(last_bill.Quote_Id[4:])
            ...
            for entry in entries:
                ...
                if last_quote:
                    # Extract the numeric part
                    last_id = int(last_quote.Item_Id[12:])
                    new_id = f"ITM{last_id + 1:01d}-{detail_id}"
                else:
                    new_id = f"ITM1-{detail_id}"
            ...
            return redirect("branchapp_one:listquote")
        except json.JSONDecodeError:
            return JsonResponse({"error": "Invalid JSON data"}, status=400)
        except Exception as e:
            traceback.print_exc()  # This prints the full error traceback in the console
...
def estimate(request):
    ...
    if request.method == "POST":
        try:
            ...
            # Save each entry into the database
            for entry in entries:
                last_bill = EstimateMaster.objects.order_by(
                    '-Estimation_Id').first()
                if last_bill:
                    # Extract the numeric part
                    last_id = int(last_bill.Estimation_Id[4:])
                ...
                break
            for entry in entries:
                last_bill = EstimateDetails.objects.order_by(
                    '-Item_Id').first()
                if last_bill:
                    # Extract the numeric part
                    last_id = int(last_bill.Item_Id[12:])
                ...
                EstimateDetails.objects.create(
                    ...
                    Customer_Name=data.get("customer_name"),
                    Phone_No=data.get("customer_phone"),
                    Product_Name=entry.get("product", "NONE") if entry.get(
                        "product") not in [None, "", "None"] else "NONE",
                    Custom_Product=entry.get("custom", "NONE") if entry.get(
                        "custom") not in [None, "", "None"] else "NONE",
                    Category_Name=entry.get("category", "NONE") if entry.get(
                        "category") not in [None, "", "None"] else "NONE",
                    Sub_Category=entry.get("sub_category", "NONE") if entry.get(
                        "sub_category") not in [None, "", "None"] else "NONE",
                    Length=int(entry.get("length", 0)) if entry.get(
                        "length") not in [None, "", "None"] else 0,
                    Height=int(entry.get("width", 0)) if entry.get(
                        "width") not in [None, "", "None"] else 0,
                    Total_Sqft=int(entry.get("total_sqft", 0)) if entry.get(
                        "total_sqft") not in [None, "", "None"] else 0,
                    # Convert to int (default 0 if missing)
                    Quantity=int(entry.get("quantity", 0)),
                    Cost_Per_Sqft=float(entry.get("cost", 0)) if entry.get("cost") not in [
                        # Convert to float (default 0 if missing)
                        None, "", "None"] else 0,
                    Total_Cost=float(entry.get("total", 0)
                                     ),  # Convert to float
                    # Total_Cost_With_Gst=float(entry.get("total_with_gst", 0))  # Convert to float
                )
            return redirect("branchapp_one:listestimate")
        except json.JSONDecodeError:
            return JsonResponse({"error": "Invalid JSON data"}, status=400)
        except Exception as e:
            traceback.print_exc()  # This prints the full error traceback in the console
    ...
    return redirect('branchapp_one:bill')
...
def add_employee(request):
    if request.method == 'POST':
        ...
        return redirect('branchapp_one:addemployee')  # Redirect after successful creation
    last_emp = Employee.objects.order_by('-emp_id').first()
    if last_emp:
        last_emp = int(last_emp.emp_id[3:])  # Extract the numeric part
        new_id = f"EMP{last_emp + 1:04d}"
...
def invoice(request, id):
    ...
    if id.startswith("BILL"):
        ...
        # Convert QR to image
        img = qr.make_image(
            fill="black", back_color="white").resize((400, 400))
        ...
        current_date = datetime.today()
        context = {'data': data, 'data2': data2,
                   'data3': data3, 'current_date': current_date}
...
def add_payment(request, id):
    ...
    if request.method == "POST":
        try:
            ...
            last_pay = Payment_Master.objects.filter(
                Payment_Id__startswith=id).order_by('-Payment_Id').first()

            if last_pay:
                # Extract the numeric part
                last_id = int(last_pay.Payment_Id[13:])
            ...
            paym_id = Payment_Master.objects.create(
                ...
                Grand_Total=int(float(data.get("totalAmount") or 0)),
                Paid_Amount=int(float((data.get("totalAmount") or 0)) -
                                int(float((data.get("finalPending") or 0)))),
            ...
            BillingMaster.objects.filter(Bill_Id=id.split("-")[0]).update(
                # Fully paid if no pending amount
                Fully_Paid=1 if data.get("finalPending") == "0.00" else 0,
                Partialy_Paid=1 if 0 < int(float((data.get("finalPending") or 0))) < int(
                    # Partial payment
                    float((data.get("totalAmount") or 0))) else 0,
                Not_Paid=1 if (data.get("totalAmount") or 0) == (
                    # Not paid if pending = total
                    data.get("finalPending") or 0) else 0,
                Paid_Amount=int(float(data.get("totalAmount"))) -
                int(float(data.get("finalPending"))),
            ...
            for entry in table_data:
                ...
                Payment_Details.objects.create(
                    ...
                    # Ensure it's never None
                    Grand_Total=int(float(data.get("totalAmount") or 0)),
                    # Avoid NoneType subtraction
                    Paid_Amount=int(float(entry.get("amount"))),
                    # Ensure correct pending amount
                    Pending_Amount=int(
                        float(entry.get("pending_amount") or 0)),
                ...
                if entry.get("payment_mode") == "MANUAL CLOSE":
                    BillingMaster.objects.filter(Bill_Id=id.split("-")[0]).update(
                        # Set Force_Paid if MANUAL_CLOSE
                        Force_Paid=1 if entry.get(
                            "payment_mode") == "MANUAL CLOSE" else 0,
                    )
            ...
            if today_expences:
                ...
                Cash_Book_Master.objects.filter(Date=current_date).update(
                    Cash_In=today_income + incomming_amount)
            else:
                last_cash = Cash_Book_Master.objects.order_by('-S_No').first()
                if last_cash:
                    # Extract the numeric part
                    last_id = int(last_cash.Expenses_Id[3:])
...
def bill_and_pay(request, id):
    ...
    if request.method == "POST":
        try:
            ...
            return redirect("branchapp_one:addpayment", id=id)
        except json.JSONDecodeError:
            return JsonResponse({"error": "Invalid JSON data"}, status=400)
        except Exception as e:
            traceback.print_exc()  # This prints the full error traceback in the console
...
def filtered_data(request, filter_type):
    ...
    next_month = start_of_month.replace(
        day=28) + timedelta(days=4)  # Jump to next month
    ...
    if filter_type == 'today':
        ...
        data = Payment_Details.objects.filter(
            Payment_Date__range=(start_of_week, end_of_week))
        # data2 = BillingMaster.objects.filter(
        # Date__range=(start_of_week, end_of_week)
        # ).exclude(Grand_Total=F('Pending_Amount'))
        data3 = BillingMaster.objects.filter(
            Date__range=(start_of_week, end_of_week))
        data4 = "This Week"
        ...
        try:
            ...
            for entry in entries:
                ...
                if bill_master.Pending_Amount == 0:
                    BillingMaster.objects.filter(Bill_Id=id).update(
                        Fully_Paid=0, Partialy_Paid=1)

                detail_id = BillingMaster.objects.filter(Bill_Id=id).update(
                    Date=data.get("bill_date"),
                    Grand_Total=existing_total +
                    int(data.get("grand_total", 0)) if data.get(
                        "grand_total") not in [None, "", "None"] else 0,
                    Grand_Total_With_Gst=0,
                    Added_By=request.user.username,
...
def graph(request):
    ...
    day_payments_totals = {entry['day'].day: entry['total']
                           for entry in daily_payments_totals}

    # Build the final list with all days (1 to num_days)
    result = {f'day{day}': float(day_totals.get(day, 0))
              for day in range(1, num_days + 1)}
    payment_result = {f'day{day}': float(
        day_payments_totals.get(day, 0)) for day in range(1, num_days + 1)}
...
def Cash_Book(request):
    if request.method == 'POST':
        try:
            ...
            today_expences = Cash_Book_Master.objects.filter(
                Date=current_date).update(
                Cash_In=available_amount - total_amount,
                Cash_Out=id_for_details.Cash_Out + total_amount)
            ...
            for entry in entries:
                ...
                for item in category:
                    if item.Expense_Name != entry.get('expense', "N/A"):
                        Expense_Category.objects.create(
                            Expense_Name=entry.get('expense', "N/A"))
...
def load_cash(request):
    if request.method == "POST":
        ...
        return redirect('branchapp_one:cashbook')
...
def add_gst_quotation(request):
    if request.method == 'POST':
        ...
        return redirect('branchapp_one:listgstquotations')

    customers = CustomerDetails.objects.filter(Status=1)
    return render(request, 'add_gst_quotation.html', {'customers': customers, 'branch_url': 'branchapp_one'})
...
def gst_quotation_pdf(request, quotation_no):
    ...
    total_in_words = "" # You might need a num2words function here if needed

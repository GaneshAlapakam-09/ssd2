...
from .models import (
    ...
    GSTQuotationMaster, GSTQuotationDetails, DeliveryChallanMaster, DeliveryChallanDetails
)
...
def add_gst_invoice(request):
    if request.method == 'POST':
        ...
        return redirect('ssdapp:listgstinvoices')

    customers = CustomerDetails.objects.filter(Status=1)
    return render(request, 'add_gst_invoice.html', {'customers': customers})
...
def add_delivery_challan(request):
    if request.method == 'POST':
        ...
        return redirect('ssdapp:listdeliverychallans')

    customers = CustomerDetails.objects.filter(Status=1)
    return render(request, 'add_delivery_challan.html', {'customers': customers, 'branch_url': 'ssdapp'})
